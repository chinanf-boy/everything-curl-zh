...
当启用日志模式时,CURL变得更加健谈,并将解释和显示更多的行为.它将添加信息测试并用"\*'.例如,让我们看看在尝试一个简单的HTTP示例(将下载的数据保存在名为"saved"的文件中)时,curl可能会说什么:
...
该选项以高分辨率定时器将所有日志的/跟踪输出输出到打印行时.它与规则一致.`-v / --verbose`选项以及`--trace`和`--trace-ascii`.
...
当然,日志的反义词是使cURL变得更安静.与`-s`(或)`--silent`选项使cURL开关关闭进度表,不输出任何错误消息,当出现错误时.它变得哑巴了.它仍然会输出你要求下载的数据.
