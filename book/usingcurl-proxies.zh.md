...
如果检查浏览器的网络设置,有时在高级设置选项卡下,可以了解浏览器配置为使用什么代理或代理.当你使用卷发时,你应该使用同一个或多个.
...
HTTP代理是客户端用HTTP进行传输的代理.默认情况下,卷发将假定您指向的主机.`-x`或`--proxy`是一个HTTP代理,除非您还指定了端口号,否则它将默认为端口3128(特定端口号的原因纯粹是历史的).
...
如果启用冗长模式`-v`当与代理对话时,您将看到curl连接到代理而不是远程服务器,并且您将看到它使用稍微不同的请求行.
